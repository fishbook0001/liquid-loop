...
def value_decay_entropy(state: WorkspaceState) -> float:
    """锚点价值衰减熵。0=全部高价值, 1=全部低价值"""
    if not state.anchors:
        return 0.0
    for a in state.anchors:
        a.decay_value()
    scores = [a.value_score for a in state.anchors]
    return 1.0 - (sum(scores) / len(scores))


def strength_entropy(state: WorkspaceState) -> float:
    """锚定强度熵。0=全部强锚定, 1=全部弱锚定"""
    if not state.anchors:
        return 0.0
...
def retrospective_decay_entropy(state: WorkspaceState) -> float:
    ...
    测量逻辑：检查 evidence 时间序列，较新的证据是否导致锚点 value_score 降低。
    每锚点取最新 vs 次新两条证据的时间差 & value_score 变化比。
    """
    ...
    for a in state.anchors:
        ...
        # 模拟"少一条证据"时的 value_score
        old_score = a.decay_value(evidence_count=len(evs) - 1)
        if old_score > 0:  # 避免除零
            ratio = max(0, 1 - new_score / old_score)
...
def behavioral_drift_entropy(state: WorkspaceState) -> float:
    ...
    测量逻辑：锚点的 stability 在最近一次更新中的变化幅度。
    """
    if not state.anchors:
        return 0.0
    drifts = []
